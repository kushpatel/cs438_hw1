...
#	exit 1
	...
	#while [ $BSIZE -lt 241 ]; do
		~/pgsql/bin/pg_ctl start -D ~/pgdata -l log.log -o "-B $buffSize -N 8 -o '-te -fm -fh'"
		sleep 1s
		for i in `seq 1 3`; do
			...
			./stripper.pl pre post > $buffSize.Clock.$i
			./stripper.pl pre post > $buffSize.LRU.$i
			./stripper.pl pre post > $buffSize.MRU.$i
		done
		#$buffSize+=16
	#done
